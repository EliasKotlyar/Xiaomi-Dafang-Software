...
#if defined(CONFIG_JZ_INTERNAL_CODEC_V12)
	destroy_workqueue(i2s_dev->i2s_work_queue);
#endif

	destroy_workqueue(i2s_dev->i2s_work_queue_1);
...
static int i2s_exit(struct platform_device *pdev)
{
	...
	if(g_i2s_dev->refcnt == 0){
		while(waitqueue_active(&switch_data.wq)){
			wake_up_interruptible(&switch_data.wq);
			mdelay(2);
		}
		platform_device_unregister(&xb47xx_i2s_switch);
