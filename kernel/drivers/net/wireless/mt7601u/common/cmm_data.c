...
//#endif // CONFIG_AP_SUPPORT //
	...
	while (1)
	{
		...
		*/
		...
		{
			printk("ACTION COME-IN !! TYPE = %d, IsReg = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
				  pHeader->FC.SubType, pAd->Cfg80211RegisterActionFrame, PRINT_MAC(pHeader->Addr2));

			CFG80211DRV_PrintFrameType(pAd, "RX", pHeader,  pRxWI->RxWIMPDUByteCnt);
		}
#if 0		
		...
		if ( (pHeader->FC.Type == BTYPE_MGMT) && (pHeader->FC.SubType == SUBTYPE_DEAUTH) )
                {
                        printk("Deauth: %02x:%02x:%02x:%02x:%02x:%02x, %02x:%02x:%02x:%02x:%02x:%02x\n",
		...
		{
			printk("NULL_FRAME: %02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pHeader->Addr2));
		}
