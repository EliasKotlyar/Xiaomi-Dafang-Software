...
struct bcm2079x_dev {
	...
	void (*power_down_gpio_set)(void);
	void (*power_on_gpio_set)(void);
};
					...
					unsigned int cmd, unsigned long arg)
{
	...
	case BCMNFC_POWER_CTL:
		dev_dbg(&bcm2079x_dev->client->dev,
			 ...
			 arg);
		if (arg == 1) {
			if (bcm2079x_dev->power_on_gpio_set) {
				bcm2079x_dev->power_on_gpio_set();
			}
			ret = bcm_power_on();
			if (ret < 0) {
				pr_err("%s, bcm_power_on error\n",__func__);
			...
			ret = bcm_power_down();
			if (ret < 0) {
				pr_err("%s, bcm_power_down error\n",__func__);
				return ret;
			}

			gpio_set_value(bcm2079x_dev->en_gpio, 0);
			if (bcm2079x_dev->power_down_gpio_set) {
				bcm2079x_dev->power_down_gpio_set();
			}
		}
		break;
...
/* hli del end */
	...
	bcm2079x_dev->power_down_gpio_set = platform_data->power_down_gpio_set;
	bcm2079x_dev->power_on_gpio_set = platform_data->power_on_gpio_set;
...
//Peter modify end
	...
	/* hli del start
	   ...
	   if (ret < 0) {
	   pr_err("%s : not able to set bcm2079x_nfc_int as input\n", __func__);
	   goto err_en;
	   }
	   hli del end  */
...
#if 0
      ret = request_irq(client->irq, bcm2079x_dev_irq_handler,
                        IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND, client->name, bcm2079x_dev);
#endif
...
static int bcm2079x_suspend(struct i2c_client *client, pm_message_t mesg)
{
	return 0;
}
static int bcm2079x_resume(struct i2c_client *client)
{
	return 0;
}
...
#if 0
	.suspend = bcm2079x_suspend,
	.resume = bcm2079x_resume,
#endif
};
