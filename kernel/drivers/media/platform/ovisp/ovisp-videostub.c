...
static void vivi_fillbuff(struct vivi_dev *dev, struct vivi_buffer *buf)
{
	...
	snprintf(str, sizeof(str), " %dx%d, input %d, rate %d ",
			dev->width, dev->height, dev->input, dev->rate);
	gen_text(dev, vbuf, line++ * 16, 16, str);
...
static void vivi_sleep(struct vivi_dev *dev)
{
	...
	timeout = msecs_to_jiffies(rate_to_ms(dev->rate));
				    ...
				    struct v4l2_control *c)
{
	...
	case V4L2_CID_BRIGHTNESS:
		dev->brightness->cur.val = c->value;
		break;
	case V4L2_CID_EXPOSURE:
		break;
	case V4L2_CID_CONTRAST:
		dev->contrast->cur.val = c->value;
		break;
	case V4L2_CID_SATURATION:
		dev->saturation->cur.val = c->value;
		break;
			...
			struct v4l2_streamparm *parm)
{
	...
	dev->rate = parm->parm.capture.timeperframe.denominator;
			...
			struct v4l2_streamparm *parm)
{
	...
	parm->parm.capture.timeperframe.denominator = dev->rate;
...
static int __init vivi_create_instance(int inst)
{
	...
	dev->v4l2_dev.ctrl_handler = NULL;//hdl;
	...
	dev->alloc_ctx = OVISP_vb2_init_ctx(NULL);
