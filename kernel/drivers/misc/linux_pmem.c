...
static int pmem_map_pfn_range(int id, struct vm_area_struct *vma,
			      ...
			      unsigned long len)
{
	DLOG("map offset %lx len %lx start %lx\n", offset, len, vma->vm_start);
...
//		dmac_flush_range(vaddr, vaddr + pmem_len(id, data));
		dma_cache_sync(NULL, vaddr, pmem_len(id, data), DMA_BIDIRECTIONAL);
		goto end;
	}
...
//			dmac_flush_range(flush_start, flush_end);
			dma_cache_sync(NULL, flush_start, region_node->region.len, DMA_BIDIRECTIONAL);
			break;
		}
	}
end:
	       ...
	       int (*release)(struct inode *, struct file *))
{
	...
	if (err) {
		printk("Unable to register pmem driver!\n");
...
//		pmem[id].vbase = ioremap_cached(pmem[id].base, pmem[id].size);
		pmem[id].vbase = ioremap_cachable(pmem[id].base, pmem[id].size);
/**
 * ioremap_ext_buffered can not support
 */
#ifdef PMEM_BUFFER
	else if (pmem[id].buffered)
...
MODULE_AUTHOR("liangbohu, <bohu.liang@ingenic.com>");
